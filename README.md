...
- **`active_custody_days` is the no-overlap constraint.** One row per day covered by an active override, primary-keyed on `(family_id, day)` — this is what guarantees two overlapping ranges can never both be active, even under concurrent approvals (the old partial unique index only caught identical start dates). Rows are written and cleared in the same transaction as the `is_active` flip via `database/activation.py`, backfilled at boot for overrides activated before the table existed, and a 409 "Conflicts with an override that was just approved" means this constraint fired.
